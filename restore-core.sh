#!/bin/bash
# Restores the known-good, tested versions of every core source file
# Run this with: bash restore-core.sh   (from your project root)
set -e

mkdir -p "server"
cat > "server/index.js" << 'RESTORE_EOF'
...
mkdir -p "src/lib"
cat > "src/lib/tokens.js" << 'RESTORE_EOF'
export const TOKENS = {
  blueprint: '#16303D',
...
mkdir -p "src/lib"
cat > "src/lib/prompts.js" << 'RESTORE_EOF'
...
mkdir -p "src/lib"
cat > "src/lib/storage.js" << 'RESTORE_EOF'
...
cat > "src/components/Diagram.jsx" << 'RESTORE_EOF'
...
mkdir -p "src/components"
cat > "src/components/QuestionCard.jsx" << 'RESTORE_EOF'
import React, { useState } from 'react';
...
mkdir -p "src/components"
cat > "src/components/ExploreTab.jsx" << 'RESTORE_EOF'
import React, { useState } from 'react';
...
mkdir -p "src/components"
cat > "src/components/ReviewTab.jsx" << 'RESTORE_EOF'
import React, { useState } from 'react';
...
mkdir -p "src/components"
cat > "src/components/PracticeTab.jsx" << 'RESTORE_EOF'
...
mkdir -p "src/components"
cat > "src/components/SavedTab.jsx" << 'RESTORE_EOF'
import React, { useState, useEffect } from 'react';
...
mkdir -p "src/components"
cat > "src/components/DashboardTab.jsx" << 'RESTORE_EOF'
...
mkdir -p "src/components"
cat > "src/components/ConnectionsTab.jsx" << 'RESTORE_EOF'
import React, { useEffect, useState } from 'react';
...
echo "All core files restored to the known-good version."
